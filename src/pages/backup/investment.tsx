...
export function Investment() {
  ...
  const { futureValue, totalContributions, totalInterest } = calculateInvestmentGrowth();
  ...
  useEffect(() => {
    setCalculatorData({
      ...
      goalAnalysis: goals.map(goal => ({
        ...
        projectedShortfall: goal.target - futureValue,
      })),
    });
  }, [investmentDetails, goals, futureValue, totalContributions, totalInterest, setCalculatorData]);
  ...
  const getRecommendedAllocation = () => {
    ...
    const adjustedStockAllocation = baseStockAllocation + timeHorizonAdjustment;
    ...
    <div className="grid gap-6 lg:grid-cols-2">
      ...
      <div className="space-y-6">
        <InvestmentSummary
          ...
          goals={goals}
          annualReturn={investmentDetails.annualReturn}
        />
        <Card className="p-6">
          <InvestmentChart
            initialAmount={investmentDetails.initialAmount}
            monthlyContribution={investmentDetails.monthlyContribution}
            annualReturn={investmentDetails.annualReturn}
            years={investmentDetails.investmentPeriod}
            compoundingFrequency={investmentDetails.compoundingFrequency}
            goals={goals}
          />
        </Card>
        <Card className="p-6">
          ...
          <div className="space-y-4 text-sm">
            ...
            <div>
              ...
              <div className="space-y-2">
                <ul className="space-y-1 text-muted-foreground">
                  <li>• Projected value: ${futureValue.toFixed(2)}</li>
                  <li>• Total contributions: ${totalContributions.toFixed(2)}</li>
                  <li>• Investment earnings: ${totalInterest.toFixed(2)}</li>
